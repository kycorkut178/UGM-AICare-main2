...
# app.include_router(aika_router)  # NEURA-AI Meta-Agent orchestrator - REMOVED (Legacy)
...
async def lifespan(app: FastAPI):
    ...
    # Compile the NEURA-AI agent ONCE here so every request reuses the same
    ...
    try:
        ...
        if _checkpointer is None:
            ...
            logger.warning(
                "NEURA-AI agent using in-memory MemorySaver "
        ...
        startup_log("NEURA-AI agent compiled and cached on app.state.aika_agent")
    except Exception:
        logger.error(
            "NEURA-AI agent compilation failed at startup (non-blocking — requests will fail until fixed)",
            exc_info=True,
        )
...
app = FastAPI(
    title="NEURA-AI Chatbot API", 
    description="API for NEURA-AI Chatbot - UGM AI Care. Uses FastAPI.",
    version="0.1",
...
# app.include_router(aika_router)  # NEURA-AI Meta-Agent orchestrator
app.include_router(aika_stream.router, prefix="/api/v1")  # NEURA-AI Streaming Endpoint
...
async def root():
    ...
    return {
        "message": "Welcome to the NEURA-AI Chatbot API!",
        "documentation": {
            "swagger_ui": "/docs",
            "redoc": "/redoc"
        },
...
async def health_check():
    ...
    return {
        ...
        "version": "0.1",
        "description": "NEURA-AI Chatbot API - UGM AI Care",
        "api_version": "v1",
        "api_base_url": "/api/v1",
