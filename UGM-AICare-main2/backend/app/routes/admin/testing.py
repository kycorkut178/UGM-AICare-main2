...
async def simulate_real_chat(
    ...
    This endpoint simulates how a real user would interact with NEURA-AI:
    ...
    # System prompt
    system_prompt = """Kamu adalah NEURA-AI, AI pendamping kesehatan mental dari NEURA-K3. Anggap dirimu sebagai teman dekat bagi mahasiswa UGM yang sedang butuh teman cerita. Gunakan bahasa Indonesia yang santai dan kasual (gaya obrolan sehari-hari), jangan terlalu formal, kaku, atau seperti robot. Buat suasana ngobrol jadi nyaman dan nggak canggung (awkward). Sebisa mungkin, sesuaikan juga gaya bahasamu dengan yang dipakai pengguna.
...
Tujuan utamamu adalah menjadi pendengar yang baik, suportif, hangat, dan tidak menghakimi. Bantu pengguna mengeksplorasi perasaan mereka terkait kehidupan kuliah, stres, pertemanan, atau apapun yang ada di pikiran mereka. Validasi emosi mereka, tunjukkan kalau kamu paham dan peduli."""
    ...
    for idx, user_message in enumerate(request.user_messages):
        ...
        try:
            ...
            agent_routing_log.append(f"Msg {idx+1}: NEURA-AI -> Direct Response") # Placeholder
            ...
            if result.intervention_plan:
                ...
                agent_routing_log[-1] = f"Msg {idx+1}: NEURA-AI -> STA -> Intervention"
            ...
            try:
                ...
                if "CMA" in agents_invoked:
                    ...
                    actual_next_agent = "TCA"
                     ...
                     actual_next_agent = "NEURA-AI" # STA -> NEURA-AI
