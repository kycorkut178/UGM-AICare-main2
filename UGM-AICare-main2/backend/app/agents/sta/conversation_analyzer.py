...
async def analyze_conversation_risk(
    ...
    if not conversation_history and not current_message:
        raise ValueError("Cannot analyze empty conversation - both history and current_message are missing")
    ...
    conversation_text = "\n\n".join([
        f"{'User' if msg['role'] == 'user' else 'NEURA-AI'}: {msg['content']}"
    ...
    analysis_prompt = f"""
You are a clinical mental health analyst reviewing a complete conversation between a university student and NEURA-AI (AI mental health assistant).
...
4. USER CONTEXT: Extract specific information about:
   - recent_stressors: What problems/challenges are they facing? (CRITICAL: Use concise keywords, max 2-3 words per item, e.g., "Academic Pressure", "Family Conflict")
