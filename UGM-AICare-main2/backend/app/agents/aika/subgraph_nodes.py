"""NEURA-AI orchestrator — sub-agent execution nodes.
...
Public API used in the LangGraph graph:
    ...
    synthesize_final_response  Compose the final NEURA-AI reply from agent outputs.
...
def _build_synthesis_prompt(
    ...
    sections = [
        "You are NEURA-AI. Synthesize a final response based on the specialized agent outputs.",
        "",
    ...
    sections.append(
        ...
        "4. Maintains NEURA-AI's personality (caring, supportive)\n"
...
async def execute_sca_subgraph(
    ...
    if execution_id:
        execution_tracker.start_node(execution_id, "neura-ai::sca", "neura-ai")
    ...
    try:
        ...
        if execution_id:
            execution_tracker.fail_node(execution_id, "neura-ai::sca", str(exc))
...
async def execute_sda_subgraph(
    ...
    execution_id = state.get("execution_id")
    if execution_id:
        execution_tracker.start_node(execution_id, "neura-ai::sda", "neura-ai")
    ...
    try:
        ...
        state.setdefault("errors", []).append(error_msg)
        if execution_id:
            execution_tracker.fail_node(execution_id, "neura-ai::sda", str(exc))
...
async def parallel_crisis_node(
    ...
    execution_id = state.get("execution_id")
    if execution_id:
        execution_tracker.start_node(execution_id, "neura-ai::parallel_crisis", "neura-ai")
    ...
    try:
        ...
        cma_result = await execute_sda_subgraph(cast(AikaOrchestratorState, cma_input), db)
        ...
        state.setdefault("errors", []).append(error_msg)
        if execution_id:
            execution_tracker.fail_node(execution_id, "neura-ai::parallel_crisis", str(exc))
...
async def execute_ia_subgraph(
    ...
    execution_id = state.get("execution_id")
    if execution_id:
        execution_tracker.start_node(execution_id, "neura-ai::ia", "neura-ai")
    ...
    try:
        ...
        state.setdefault("errors", []).append(error_msg)
        if execution_id:
            execution_tracker.fail_node(execution_id, "neura-ai::ia", str(exc))
...
async def synthesize_final_response(
    ...
    """Compose the final NEURA-AI reply when a sub-agent has run.
    ...
    execution_id = state.get("execution_id")
    if execution_id:
        execution_tracker.start_node(execution_id, "neura-ai::synthesize", "neura-ai")
    ...
    try:
        ...
        if state.get("aika_direct_response"):
            ...
            if execution_id:
                execution_tracker.complete_node(execution_id, "neura-ai::synthesize")
            return state
        ...
        if execution_id:
            execution_tracker.fail_node(execution_id, "neura-ai::synthesize", str(exc))
...
async def execute_sta_subgraph(
    ...
    execution_id = state.get("execution_id")
    if execution_id:
        execution_tracker.start_node(execution_id, "neura-ai::sta", "neura-ai")
    ...
    try:
        ...
        if execution_id:
            execution_tracker.fail_node(execution_id, "neura-ai::sta", str(exc))
