"""NEURA-AI decision node — first node of the unified LangGraph orchestrator.
...
and either route to it or generate a direct NEURA-AI response via the ReAct loop.
...
async def _generate_direct_response(
    ...
    """Generate a direct NEURA-AI response when no sub-agent is needed (ReAct loop).
...
async def aika_decision_node(
    ...
    """First node of the unified NEURA-AI orchestrator graph.
    ...
    if execution_id:
        execution_tracker.start_node(execution_id, "neura-ai::decision", "neura-ai")
    ...
    try:
        ...
        if normalized_role == "user" and _is_smalltalk_message(current_message):
            ...
            logger.info(
                "NEURA-AI Decision (deterministic smalltalk): duration=%.0fms", elapsed_ms
            )
        ...
        logger.info("NEURA-AI decision using model: %s", preferred_model)
        ...
        logger.info(
            "NEURA-AI Decision: intent=%s, needs_agents=%s, duration=%.0fms",
            state.get("intent"),
        ...
        error_str = str(exc)
        error_msg = "NEURA-AI decision node failed: %s" % error_str
        ...
        if is_rate_limit:
            ...
            if execution_id:
                execution_tracker.complete_node(
                    execution_id, "neura-ai::decision", metrics={"fallback": "rate_limit"}
                )
        else:
            ...
            if execution_id:
                execution_tracker.fail_node(execution_id, "neura-ai::decision", error_str)
