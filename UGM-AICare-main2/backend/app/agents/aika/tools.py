"""Tool definitions and handlers for NEURA-AI's function calling capabilities.
...
def get_aika_tools(
    ...
    """Return list of tools available to NEURA-AI.
    ...
    try:
        ...
        if allowed_tool_names is None:
            logger.info(
                "✅ Loaded %d tool groups for NEURA-AI role=%s (%d declarations)",
                len(filtered_tools),
                normalized_role,
            ...
            logger.info(
                "✅ Loaded %d tool groups for NEURA-AI role=%s (%d declarations after role+intent allowlist)",
                len(filtered_tools),
                normalized_role,
                selected_declarations_count,
            )
        ...
        logger.error("Error loading NEURA-AI tools: %s", e)
        return []
