...
class ExecutionStateTracker:
    ...
    def record_decision_parse_outcome(
        ...
        *,
        ...
        """Record NEURA-AI decision-parse quality metrics and trigger threshold alerts."""
    ...
    async def _check_decision_parse_failure_threshold(self, *, days: int = 1) -> None:
        ...
        try:
            ...
            await self._create_alert(
                alert_type="quality",
                severity=severity,
                title="NEURA-AI Decision Parse Failure Rate Elevated",
                message=(
    ...
    async def get_decision_parse_health(self, days: int = 7) -> Dict[str, Any]:
        """Return NEURA-AI decision JSON parse health summary for dashboard/alerts."""
        try:
            async with AsyncSessionLocal() as db:
                cutoff_date = datetime.now() - timedelta(days=days)
