...
export interface RiskAssessment {
  ...
  next_step: 'tca' | 'cma' | 'end';  // Routing decision (cma = SDA/CMA escalation path)
}
...
export interface CMAGraphRequest {
  ...
  intent: string;
  risk_score: number;          // 0.0-1.0 normalized score
  triage_assessment_id: number | null; // Database ID of triage assessment
}
...
export interface IAGraphRequest {
  question_id: string;         // One of allow-listed queries
  start_date: string;          // ISO date
  end_date: string;            // ISO date
  user_hash: string;           // Anonymized analyst identifier
...
export interface ExecutionHistoryFilters {
  ...
  graph_name?: 'sta' | 'orchestrator' | 'cma' | 'tca' | 'ia' | 'neura-ai';
}
