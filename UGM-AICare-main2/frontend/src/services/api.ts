...
apiClient.interceptors.response.use(
  ...
  (error) => {
    if (error.response?.status === 401) {
      logger.error(`API request Unauthorized (401): ${error.response.data?.detail}`);
      ...
      if (axios.isAxiosError(error)) {
        ...
        const msg = `API Error (${status}): ${JSON.stringify(error.response?.data) || error.message}`;
...
export const getActiveJournalPrompts = async (): Promise<JournalPromptResponse[]> => {
  try {
    ...
    logger.error('Error fetching journal prompts', error);
