 ...
 * useAika Hook
 * 
 * React hook for interacting with the NEURA-AI Meta-Agent orchestrator.
 * This replaces direct agent calls with a unified LangGraph-orchestrated interface.
 * 
 * Features:
 * - Unified API endpoint (/api/v1/neura-ai)
...
function buildAikaEndpoint(): string {
  ...
  return apiOrigin ? `${apiOrigin}/api/v1/neura-ai` : '/api/v1/neura-ai';
}
...
export function useAika(options: UseAikaOptions = {}) {
  ...
  /**
   * Send a message to NEURA-AI Meta-Agent over an SSE stream.
   *
  ...
  const sendMessage = useCallback(async (
    ...
    try {
      ...
      if (!finalMetadata) {
        throw new Error('Incomplete response from NEURA-AI');
      }
      ...
      console.error('NEURA-AI API Error:', err);
      return null;
    } finally {
      setLoading(false);
    }
