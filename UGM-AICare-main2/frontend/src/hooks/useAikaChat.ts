 ...
 * useAikaChat Hook
 * 
 * Enhanced chat hook that integrates NEURA-AI Meta-Agent orchestration
...
export function useAikaChat({ 
  ...
  const {
    ...
    onRiskDetected: (assessment) => {
      // Risk handled via UI indicators
    },
    onEscalation: (caseId) => {
      // Escalation handled via system notifications
    },
    onToolEvent: (event) => {
      ...
      setMessages([
        {
          id: greetingId,
          role: 'assistant',
          content: 'Halo! Aku NEURA-AI, asisten AI untuk kesehatan mentalmu. Bagaimana kabarmu hari ini? 💙',
          timestamp: new Date(),
          session_id: sessionId,
          conversation_id: conversationId,
          created_at: new Date().toISOString(),
  ...
  const handleSendMessage = useCallback(
    async (message?: string) => {
      ...
      setCurrentThinking('NEURA-AI sedang menganalisis pesanmu...');
      setThinkingTrace([]);
      ...
      try {
        // Prepare conversation history for NEURA-AI
        ...
        if (!aikaResponse) {
          throw new Error('Failed to get response from NEURA-AI');
        }
        ...
        console.error('NEURA-AI chat error:', error);
