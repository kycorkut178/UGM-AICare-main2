...
/**
 * Get counselor users who don't have a profile yet (Admin)
 */
export async function getCounselorUsers(): Promise<CounselorUser[]> {
  const response = await fetch(`${API_BASE}/api/v1/admin/counselors/selectable-users`, {
    headers: await getAuthHeaders(),
    credentials: 'include',
  });

  return handleResponse<CounselorUser[]>(response);
}
